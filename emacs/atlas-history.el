...
(defun atlas-history-version-diff (version)
  "Show all entities that changed in VERSION, including renames and deletions."
  (interactive
   (list (atlas--history-completing-read-version "Version: ")))
  (let* ((result (atlas--eval-safe
                  (format "(history-version-diff-full \"%s\")" version)))
         (changes (atlas--to-list (atlas--get result 'changes)))
         (deleted (atlas--to-list (atlas--get result 'deleted)))
         (renames (atlas--to-list (atlas--get result 'renames)))
    ...
    (with-current-buffer buf
      ...
      ;; Renames section
      (when renames
        (atlas--insert-subheader (format "Renames (%d)" (length renames)))
        (dolist (r renames)
          (let ((from (atlas--get r 'from))
                (to   (atlas--get r 'to))
                (type (atlas--get r 'type)))
            (insert "  ")
            (atlas--insert-entity from)
            (insert (propertize " → " 'face 'font-lock-keyword-face))
            (atlas--insert-entity to)
            (insert (format "  [%s]\n" (atlas--to-string type)))))
        (insert "\n"))
      ;; Changes section (filter out entities that are part of renames)
      (let* ((rename-to-ids (mapcar (lambda (r) (atlas--to-string (atlas--get r 'to))) renames))
             (filtered-changes
              (seq-remove (lambda (snap)
                            (member (atlas--to-string (atlas--get snap 'snap/dev-id))
                                    rename-to-ids))
                          changes)))
        (when filtered-changes
          (atlas--insert-subheader (format "Changed (%d)" (length filtered-changes)))
          (dolist (snap filtered-changes)
            (let ((dev-id  (atlas--get snap 'snap/dev-id))
                  (type    (atlas--get snap 'snap/type))
                  (added   (atlas--get snap 'snap/added))
                  (removed (atlas--get snap 'snap/removed))
              ...
              (when added
                ...
                (insert "\n"))))
          (insert "\n")))
      ;; Deleted section (filter out entities that are part of renames)
      (let* ((rename-from-ids (mapcar (lambda (r) (atlas--to-string (atlas--get r 'from))) renames))
             (filtered-deleted
              (seq-remove (lambda (snap)
                            (member (atlas--to-string (atlas--get snap 'snap/dev-id))
                                    rename-from-ids))
                          deleted)))
        (when filtered-deleted
          (atlas--insert-subheader (format "Deleted (%d)" (length filtered-deleted)))
          (dolist (snap filtered-deleted)
            (let ((dev-id (atlas--get snap 'snap/dev-id))
                  (type   (atlas--get snap 'snap/type)))
              (insert "  ")
              (atlas--insert-entity dev-id)
          ...
          (insert "\n")))
      ;; Empty state
      (when (and (null changes) (null deleted) (null renames))
